...
function find_package_in_registry(pkg,
    ...
    if haskey(registry_data.packages, uuid)
        ...
        if endswith(repo, ".git")
            tmp_repo = repo[1:end-4]
        end
        if endswith(package_repo, ".git")
            tmp_package_repo = package_repo[1:end-4]
        end
        if tmp_repo != tmp_package_repo
...
function update_deps_file(pkg,
    ...
    if isfile(deps_file)
        deps_data = load_pkg_file(deps_file)
    else
        deps_data = Dict()
    end
...
function update_compat_file(pkg,
    ...
    if isfile(compat_file)
        compat_data = load_pkg_file(compat_file)
    else
        compat_data = Dict()
    end
